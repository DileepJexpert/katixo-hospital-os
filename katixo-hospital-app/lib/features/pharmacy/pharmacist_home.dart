...
class _PharmacistHomeState extends State<PharmacistHome> {
  ...
  void initState() {
    ...
    _refreshTimer =
        Timer.periodic(const Duration(seconds: 10), (_) => _loadQueue());
  }
  ...
  Future<void> _loadQueue() async {
    try {
      ...
      if (mounted) {
        setState(() => _items =
            List<Map<String, dynamic>>.from(page['content'] as List? ?? []));
      }
    } catch (_) {
  ...
  Future<void> _priorityDialog(Map<String, dynamic> item) async {
    ...
    final apply = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text('Lower number = dispensed sooner. '
    ...
    if (apply == true && reasonCtrl.text.trim().isNotEmpty) {
      await _action(
        ...
        body: {
          'newPriority': int.tryParse(priorityCtrl.text) ?? 0,
  ...
  Future<void> _rejectDialog(Map<String, dynamic> item) async {
    ...
    if (reject == true && reasonCtrl.text.trim().isNotEmpty) {
      await _action(
        '/api/v1/pharmacy/queue-items/${item['itemId']}/reject',
                  ...
                  children: [
                    for (var i = 0; i < _items.length; i++) ...[
                      _QueueItemTile(
                        ...
                        loading: _loading,
                        onStart: () => _action(
                            '/api/v1/pharmacy/queue-items/${_items[i]['itemId']}/start'),
                        onComplete: () => _action(
                            '/api/v1/pharmacy/queue-items/${_items[i]['itemId']}/complete'),
...
class _QueueItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      title: Text('${item['medicineName']} ×${item['quantity']}'
          '${overridden ? '  ⚡' : ''}'),
      subtitle: Text(
          'Rx #${item['prescriptionId']} · patient #${item['patientId']} · '
