...
public class NursingController {
    ...
    @PostMapping("/vitals")
    @PreAuthorize("hasAnyRole('NURSE', 'ADMIN')")
    public ResponseEntity<NursingVitalService.NursingVitalResponse> recordVital(
            @RequestBody NursingVitalService.RecordVitalRequest request) {
        log.info("Recording vital signs for patient {}", request.patientId);
        var vital = vitalService.recordVital(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(vital);
    }

    @GetMapping("/vitals/admission/{admissionId}")
    @PreAuthorize("hasAnyRole('NURSE', 'DOCTOR', 'ADMIN')")
    public ResponseEntity<List<NursingVitalService.NursingVitalResponse>> getVitalHistory(
            @PathVariable Long admissionId) {
        log.info("Fetching vital history for admission {}", admissionId);
        var vitals = vitalService.getVitalHistory(admissionId);
        return ResponseEntity.ok(vitals);
    }

    @GetMapping("/vitals/abnormal")
    @PreAuthorize("hasAnyRole('NURSE_SUPERVISOR', 'DOCTOR', 'ADMIN')")
    public ResponseEntity<List<NursingVitalService.NursingVitalResponse>> getAbnormalVitals() {
        log.info("Fetching abnormal vital signs");
        var vitals = vitalService.getAbnormalVitals();
        return ResponseEntity.ok(vitals);
    }

    @PostMapping("/vitals/{vitalId}/review")
    @PreAuthorize("hasAnyRole('DOCTOR', 'ADMIN')")
    public ResponseEntity<NursingVitalService.NursingVitalResponse> reviewVital(@PathVariable Long vitalId) {
        log.info("Reviewing vital record {}", vitalId);
        var vital = vitalService.reviewVital(vitalId);
